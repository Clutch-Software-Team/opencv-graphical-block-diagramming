...
export function engine_run (node_List,cv){
    ...
    function runNode (node, listId,cv){
        ...
        if(node.isExecuted == false){
            ...
            if(node.type == "start"){
                var f = new Function('node,node_id,cv' , ' console.log("start ici node_id: "+node_id); \
                ...
                let refNodeName = isReferanceParam(node.data.parameters[0].currentValue);
        
                let i = nodeIndexBul(refNodeName);
                if(nodeList[i].isExecuted == false){
                    if(!runNode(nodeList[i],i,cv)){console.log(refNodeName + " nodu calistirilamadi."); return false;}
                }
                console.log("168 var i:" + i + " refNodeName: "+ refNodeName);
                var f = new Function('node,cv,nodex', '\
                ...
                var f = new Function('i,cv,nodeList', ' var mat1 = new cv.Mat();\
                ' + fonctionString + ' \
                console.log("sdsd "+name +"ar working now!" +"'+fonctionString+'");\
    ...
    function createFunction(data){
        ...
        data.parameters.forEach(element => {
            ...
            if(currVal == "" || currVal == null){
                ...
                //gerekliyse hata ver
                errorFunction(nodeList[1].data.functionName + " fonksiyonunda " + element.name + " parametresi eksik!");
                return 0;
                }else{
            ...
            //referans node ise (henuz calismadiysa calistir ve   git o nodun donus degerini al
                ...
                if( isExecuted == false){
                console.log("referansNode: "+referansNode + ", isExecuted: "+ isExecuted + " ,nodeIndexBul otuput: " +nodeIndexBul(referansNode));
                if( runNode(nodeList[nodeIndexBul(referansNode)],nodeIndexBul(referansNode),cv) == 0) {return 0;}
                }
                isExecuted = nodeCalistiMi( referansNode );
                console.log(" runNode sonrasi :referansNode: "+referansNode + ", isExecuted: "+ isExecuted + " ,nodeIndexBul otuput: " +nodeIndexBul(referansNode));
                
                fString = fString + "nodeList["+isExecuted+"].data.returnValue" + ",";
            }
            }
